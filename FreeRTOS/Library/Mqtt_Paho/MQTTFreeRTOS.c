...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		setsockopt(n->my_socket, SO_KEEPALIVESEND, (void*)xTicksToWait);
...
int NetworkConnect(Network* n, char* addr, int port)
{
	...
	uint8_t hostIp[4];
	if((retVal = DNSTimeOut((uint8_t*)addr, hostIp, NETINFO_DNS_TIMEOUT)) < 0)
	{
		goto exit;
	}

	n->pSockPool = sockPoolAlloc();
//	if((retVal = socket(n->pSockPool->sockNo, Sn_MR_TCP, port, SF_IO_NONBLOCK)) < SOCK_ERROR)
	if((retVal = socket(n->pSockPool->sockNo, Sn_MR_TCP, port, 0)) < SOCK_ERROR)
	{
		printf("socket creation fail. ret:%d\r\n", n->pSockPool->sockNo);
		sockPoolFree(n->pSockPool);
		goto exit;
	}

	n->pSockPool->sockNo = retVal;
	n->my_socket = retVal;

	if((retVal = connect(n->my_socket, hostIp, port) ) < SOCK_OK)
	{
		printf("socket connection fail. ret:%d\r\n", retVal);
		sockPoolFree(n->pSockPool);
		retVal = -1;
		goto exit;
	}
