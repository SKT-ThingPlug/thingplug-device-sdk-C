...
static int CheckAttributes(int resourceType, int operation, char* fr, char* to, void* pc) {
	...
	{
		char print_buf[256];
		snprintf(print_buf,256,"ty<%d> op<%d> fr<%s> to<%s>", resourceType, operation, fr, to);
		Stream_print_str(NULL,print_buf);
	}
...
int tp_oneM2M_v1_14_Request(int resourceType, int operation, char* fr, char* to, void* pc) {
    ...
    if(rc != TP_SDK_SUCCESS) {
		Stream_print_str(NULL,"TP_SDK_FAILURE!!");
        return rc;
    }
...
int tp_v1_14_AddData(char* data, unsigned char length) {
	...
	Stream_print_str(NULL,"tp_v1_14_AddData");
	Stream_print_str(NULL,gContent->data);
