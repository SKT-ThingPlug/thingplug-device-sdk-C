...
#ifdef SPT_DEBUG_ENABLE
	SKTtpDebugLog(LOG_LEVEL_INFO, "ty : %d, op : %d, fr : %s, to : %s", resourceType, operation, fr, to);
#else
...
#ifdef SPT_DEBUG_ENABLE
	SKTtpDebugLog(LOG_LEVEL_INFO, "GenerateRequestId : %d\n", id);
#else
    SKTDebugPrint(LOG_LEVEL_INFO, "GenerateRequestId : %d\n", id);
#endif
    return id;
}
